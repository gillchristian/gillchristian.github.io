...
[@react.component]
let make = () => {
  let (articles, setArticles) = React.useState(() => RemoteData.NotAsked);
    ...
    Js.Promise.(
      ...
      |> then_(res =>
           (
             switch (res) {
             | IO.Ok(articles) =>
               setArticles(_ => RemoteData.Success(articles))
    ...
    <div className="Site-content">
      <blockquote>
        <p>
          {
            ReasonReact.string(
              "I have only one method that I recommended extensively - it's called think before you write.",
            )
          }
        </p>
